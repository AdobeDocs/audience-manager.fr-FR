...
keywords: d_caller, d_cb, d_cid, d_cid_ic, d_coppa, d_cts=1, d_cts=2, d_tdpid, d_dst=1, d_dst_filter, d_mid, d_ptfm, d_nsid, d_rs, d_rtbd=json, d_tdpid_pid
...
# Attributs pris en charge pour [!DNL DCS] [!DNL API] Appels {#supported-attributes-for-dcs-api-calls}

Répertorie et décrit la syntaxe et les attributs pris en charge (ou paires clé-valeur) que vous pouvez transmettre à la variable [!UICONTROL Data Collection Servers] ([!DNL DCS]). Ces informations peuvent vous aider à formater votre [!DNL DCS] demande et comprend les paramètres renvoyés par ce système.
...
<table id="table_23B7E15EC13749E9A245DFB543822DB7"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><span class="keyword"> Audience Manager</span> attributs. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Attributs privés définis par le client. </p> <p> Le serveur de collecte de données accepte vos propres données privées lorsque la clé comporte une <code> p_</code> préfixe. Les données privées sont utilisées pour l’évaluation des caractéristiques, mais elles ne seront ni enregistrées dans notre système. Par exemple, supposons qu’une caractéristique soit définie comme <code> customers = p_age&lt;25</code> et vous passez <code> p_age=23</code> dans un appel d’événement. Dans ces conditions, l’utilisateur qui répond aux critères de qualification en fonction de l’âge est admissible pour la caractéristique, mais la paire clé-valeur est abandonnée après la suppression de la variable <span class="keyword"> Audience Manager</span> reçoit la demande et n’est pas consigné. </p> </td>
  </tr> 
 </tbody> 
</table>
...
Toutes ces options sont facultatives, sauf si vous souhaitez obtenir une réponse de la [!DNL DCS]. Si vous souhaitez que la variable [!DNL DCS] pour renvoyer une réponse, puis `d_rtbd=json` est obligatoire.
...
<table id="table_FCCE4F9D796648899772A191981EFDE6"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Utilisé pour identifier l’appelant qui effectue l’appel à la fonction <span class="wintitle"> DCS</span> API. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_cb</code> </p> </td> 
   <td colname="col2"> <p>Spécifie une fonction JavaScript à exécuter à l’aide de la fonction <span class="wintitle"> DCS</span> réponse en tant que paramètre de la fonction de rappel. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_cid</code> </p> </td> 
   <td colname="col2"> <p>Contient une ou plusieurs paires d’identifiants de fournisseur de données (<code> DPID</code>) et les identifiants utilisateur des fournisseurs de données (<code> DPUUID</code>) affectée par <span class="keyword"> Audience Manager</span>. Si vous utilisez plusieurs paires <code> DPID</code>s et <code> DPUUID</code>s, séparez chaque paire par le caractère non imprimable <code> %01</code>. Par exemple: <code><i>DPID</i>%01<i>DPUUUID</i></code>. </p> <p><code> d_cid</code> remplace <code> d_dpid</code> et <code> d_dpuuid</code>, qui sont obsolètes mais toujours pris en charge. Voir <a href="../../../reference/cid.md">CID remplace DPID et DPUUID</a>. </p> </td>
  </tr>
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Désactivez l’utilisation de cookies tiers pour se conformer aux réglementations de protection de l’enfant. Ce paramètre est défini dynamiquement par le service Adobe Experience Platform Identity Adobe et dépend de la variable <code> idSyncDisable3rdPartySyncing</code> configuration. Voir <a href="https://experienceleague.adobe.com/docs/id-service/using/reference/coppa.html" format="https" scope="external"> Prise en charge de la loi COPPA dans le service Adobe Experience Platform Identity</a>. </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p><code> d_cts=1</code> </p> <p><code> d_cts=2</code> </p> </td> 
   <td colname="col2"> <p>Facultatif. Activé à la demande du client. Contactez votre conseiller Adobe Audience Manager ou l’assistance clientèle. </p> <p>Indique que les caractéristiques et les segments doivent être renvoyés dans la variable <code> JSON</code> réponse. </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Renvoie les données de destination d’URL dans la variable <code> JSON</code> réponse. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Indique la variable <code> JSON</code> version à utiliser dans la réponse. Normalement, vous devez définir cette valeur sur <code> d_jsonv=1</code>. Paramètre <code> d_jsonv=0</code> désactive les synchronisations des identifiants. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_mid</code> </p> </td> 
   <td colname="col2"> <p>Indique l’ID d’Experience Cloud défini et utilisé par le <span class="keyword"> Experience Cloud</span> Service d’ID. Pour plus d’informations sur l’ECID, voir <a href="https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html" format="https" scope="external"> Cookies et service Identity Experience Cloud</a>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Obligatoire si vous souhaitez une <code> JSON</code> réponse de la <span class="wintitle"> DCS</span>. </p> <p> 
     <ul id="ul_9EA00BD822504BCA8ECB59C1634DB91A"> 
      <li id="li_7CB890F92C4A4C6AA8B4EE32E1AD4564">Si vous l’omettez, la variable <span class="wintitle"> DCS</span> renvoie un pixel dans l’en-tête. </li> 
      <li id="li_824C23B4C7AA4B5EBADF73D26016A18E">Si vous incluez ce paramètre, la variable <span class="wintitle"> DCS</span> renvoie une <code> JSON</code> dans le corps de la réponse. Voir l’exemple ci-dessous. Votre réponse pourrait être plus complexe. </li> 
     </ul> </p> <p> 
     <code class="syntax javascript">
      {
      &nbsp;&nbsp;&nbsp;&nbsp;"stuff":&nbsp;[],
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Transmet une source de données pour l’évaluation des caractéristiques. Seules les caractéristiques de cette source de données sont évaluées. </p> <p>Par exemple, supposons que vous ayez : </p> <p> 
     <ul id="ul_6230777E16C14DCB83025A101A4ECA14"> 
      <li id="li_71F3970417BC4B93881A3E12DADE4120"><b>Caractéristique T1</b> par : </li> 
     ...
     <ul id="ul_0C30A8AE349D43A08490DA76CB4B06FA"> 
      <li id="li_F1E8DB26168B471FA35D82F4DD3AC601"><b>Caractéristique T2</b> par : </li> 
...
| --- | --- | 
| `h_host` | Défini sur le nom d’hôte de collecte de données spécifique du client. Il apparaît sous la forme `host name .demdex.net`. Voir [Signification des appels vers le domaine Demdex](https://experienceleague.adobe.com/docs/audience-manager/user-guide/reference/demdex-calls.html?lang=en). |
| `h_user-agent` | Définissez sur la variable `User-Agent` valeur d’en-tête. |
| `h_accept-language` | Définissez sur la variable  `Accept-Language`  valeur d’en-tête. |
| `h_referer` | Définissez sur la variable `Referer` valeur d’en-tête. |
| `h_referrer` | Définissez sur la variable `Referrer` valeur d’en-tête. |
| `h_date` | Définissez sur la variable `Date` valeur d’en-tête. |