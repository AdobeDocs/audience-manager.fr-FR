...
| Code d’erreur | Message d’erreur | Description |
|---|---|---|
| 100 | Impossible de récupérer le nom d’hôte de la requête | Un [!DNL API] L’appel n’a pas envoyé l’hôte [!DNL HTTP] dans la requête. Ajoutez l’en-tête hôte à l’appel et réessayez. La plupart des navigateurs et [!DNL API] les clients le font automatiquement. |
| 101 | Non valide [!DNL Experience Cloud] id transmis `ID` | Le [!DNL DCS] contient un appel non valide [!DNL Experience Cloud] ID. Vérifiez les `d_mid=` paire clé-valeur dans la chaîne d’en-tête. Assurez-vous que vous transmettez le bon [!DNL Experience Cloud] Identifiant et relancez la requête. |
| 102 | Non valide [!DNL AAM ID] requête transmise `ID` | Le [!DNL DCS] contient un appel non valide [!DNL Audience Manager] ID. Vérifiez les `d_uuid=` paire clé-valeur dans la chaîne d’en-tête. Assurez-vous que vous transmettez le bon [!DNL Audience Manager] Identifiant et relancez la requête. |
...
| 109 | Référent `HTTP referer` n’est pas autorisé pour le partenaire `Partner ID` | Le `HTTP referer` L’en-tête de l’appel n’est pas autorisé pour l’ID de partenaire de l’appel. Vérifiez que la variable `HTTP referer` L’en-tête est correct. |
...
<table id="table_A50C284AB84F48A79B01223D991884A2"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Relation de confiance renforcée via <a href="https://www.networkadvertising.org/" format="http" scope="external"> NAI</a> </p> </td> 
...
<table id="table_CFF2252A3CC54960802905454A867D7A"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>renvoyée lorsque la variable <a href="../../../reference/ids-in-aam.md"> ID de client (UUID)</a> Les associés à un ID d’appareil ne peuvent pas être récupérés pour une règle de dernière fusion authentifiée à partir de notre stockage interne. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_31F1593C46804DDBA2E9BEDE83F2417F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>When <code>gdpr=1</code> et la chaîne IAB TC est générée par un ID de CMP qui n’est pas présent dans la version mise en cache de la liste globale de CMP en Audience Manager au moment de l’évaluation, le module externe d’Audience Manager pour IAB TCF ignore la chaîne IAB TC et traite la demande comme d’habitude. La macro IAB TCF v2.0 ${GDPR} est définie sur 0 et la macro ${GDPR_CONSENT_XXX} est vide.</p>
   </td>
  </tr>
   <tr> 
   <td colname="col1"> <p>314 </p> </td> 
   <td colname="col2"> <p>L’ID de CMP est marqué comme supprimé dans GCL</p> </td> 
   <td colname="col3"> <p>When <code>gdpr=1</code> et la chaîne IAB TC est générée par une CMP qui est marquée comme supprimée dans notre version mise en cache de la liste de CMP globale, le module externe d’Audience Manager pour IAB TCF ignore la chaîne TC et traite la demande comme d’habitude, si l’heure d’évaluation est postérieure à l’heure de suppression de la liste de CMP globale. La macro IAB TCF v2.0 ${GDPR} est définie sur 0 et la macro ${GDPR_CONSENT_XXX} est vide.</p></td>
  </tr>
   <tr> 
...
Le [!DNL DCS] renvoie les codes d’erreur et les messages dans un [!DNL JSON] ou dans un en-tête X dans la chaîne de réponse HTTP.
