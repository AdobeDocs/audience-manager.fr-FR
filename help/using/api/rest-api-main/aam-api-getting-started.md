...
* Utilisez une `POST` méthode pour appeler `https://aam.adobe.io/oauth/token`.
...
## Code d’autorisation et authentification implicite {#authentication-code-implicit}
 ...
 Gestionnaire de  de [!UICONTROL REST API] prend en charge le code d’autorisation et l’authentification implicite. Pour utiliser ces méthodes d’accès, vos utilisateurs doivent se connecter pour accéder `https://aam.adobe.io/oauth/authorize` aux jetons et les actualiser.
