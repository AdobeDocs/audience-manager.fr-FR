...
>[!IMPORTANT]
>
>Selon votre méthode d’authentification, vous devez ajuster les URL de votre requête en conséquence. Pour plus d’informations sur les noms d’hôtes à utiliser, reportez-vous à la section [du](#environments) .
...
Selon la méthode d’authentification que vous utilisez, vous devez ajuster les URL de votre requête en fonction des tableaux ci-dessous.
...
### URL de requête pour l’authentification OAuth (obsolète) {#request-urls-oauth}
