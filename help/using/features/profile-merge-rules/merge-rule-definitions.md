...
Les options [!UICONTROL profile merge rule] vous permettent de contrôler le type de données que [!DNL Audience Manager] utilise pour la segmentation. Un [!UICONTROL profile merge rule] peut inclure des profils de périphérique mappés par le graphique de périphérique [!UICONTROL Profile Link], le [!UICONTROL Adobe Experience Cloud Device Co-op] et/ou d&#39;autres fournisseurs de graphiques de périphériques tiers intégrés à [!DNL Audience Manager]. Vous pouvez créer un maximum de 4 [!UICONTROL Profile Merge Rules]. Le quatrième [!UICONTROL Profile Merge Rule] est disponible exclusivement pour les clients qui ont acheté le module complémentaire [!UICONTROL People-Based Destinations].
...
Selon votre configuration [!UICONTROL Profile Merge Rules], [!DNL Audience Manager] peut effectuer l&#39;évaluation [!UICONTROL segment] en temps réel, en lot ou les deux.

* L&#39;évaluation [!UICONTROL segment] en temps réel requiert [!DNL DCS] pour que les visiteurs puissent accéder à vos propriétés numériques en temps réel, pour être admissibles au [!UICONTROL segment].
* L&#39;évaluation du lot [!UICONTROL segment] est effectuée par rapport à [!UICONTROL traits] précédemment qualifié.
* [!UICONTROL Profile Merge Rules] qui prennent en charge à la fois l&#39; [!UICONTROL segment] évaluation en temps réel et l&#39; [!UICONTROL traits]évaluation par lots combinent l&#39;activité d&#39;visiteur en temps réel avec des éléments précédemment qualifiés.
...
L&#39;évaluation du lot [!UICONTROL segment] peut prendre jusqu&#39;à 24 heures pour être prise en compte dans les rapports [Règles de fusion des Profils](profile-link-metrics.md).

## [!UICONTROL Cross-Device Options] {#auth-options}

[!UICONTROL Cross-Device Options] vous permet de sélectionner des utilisateurs authentifiés et non authentifiés et d’exploiter leur profil sur plusieurs périphériques pour la segmentation. Ces options vous aident à identifier et à atteindre des utilisateurs spécifiques sur un périphérique partagé. Pour plus d’informations sur les utilisateurs anonymes et authentifiés, voir [Visiteur Authentication States in Audience Manager](../../reference/visitor-authentication-states.md).
...
<table id="table_4CE2DD312F54480E96BEAF72800789FB"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Indique à <span class="keyword"> l'Audience Manager</span> de lire et d'écrire des données au profil authentifié si un visiteur s'est connecté à votre site. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> Derniers Profils authentifiés</span></b> </p> </td> 
   <td colname="col2"> <p>Indique à <span class="keyword"> l'Audience Manager</span> de lire les données du profil authentifié de l'utilisateur qui s'est connecté pour la dernière fois sur le périphérique. </p> <p>Si cette option est sélectionnée, <span class="keyword"> Audience Manager</span> n’écrira pas de nouvelles données de caractéristiques au profil authentifié si l’utilisateur est anonyme. Lors de l’authentification, les nouvelles données de caractéristiques sont écrites dans le profil authentifié de l’utilisateur. </p> </td>
  </tr> 
  <tr> 
...
Le [!UICONTROL Cross-Device Profile Options] liste votre [!UICONTROL cross-device data sources]. Ces options utilisent les noms que vous avez indiqués lorsque vous avez créé une [!UICONTROL cross-device] [!UICONTROL data source] (voir [Création d’une source de données sur plusieurs périphériques](merge-rules-start.md#create-data-source)). Vous pouvez sélectionner jusqu’à 3 [!UICONTROL cross-device data sources] à utiliser avec chaque règle de profil. Les [!UICONTROL Authenticated Profile Options] sont disponibles lorsque vous sélectionnez **[!UICONTROL Current Authenticated Profiles]** ou **[!UICONTROL Last Authenticated Profiles]**.

## [!UICONTROL Device Options] {#device-options}

Le [!UICONTROL Device Options] vous permet de sélectionner le type de *`device profile`* utilisé par un [!UICONTROL Profile Merge Rule]. Un profil de périphérique est créé à partir de [!UICONTROL traits] collecté à partir d’une activité de navigation anonyme. Au minimum, un [!UICONTROL profile merge rule] comprend un [!UICONTROL authenticated option] et un [!UICONTROL device option].
...
<table id="table_D373FB787D1A4E3485C02C4A76F03395"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Indique à <span class="keyword"> l'Audience Manager</span> de lire les profils du périphérique actuel et jusqu'à 100 autres périphériques à partir desquels l'utilisateur s'est authentifié. Ce graphique de périphérique est construit sur vos propres données propriétaires dans <span class="keyword"> Audience Manager</span>. Il est idéal pour les clients qui disposent d’un niveau élevé d’authentification sur l’ensemble de leurs propriétés numériques. Le graphique de périphérique <span class="wintitle"> Lien de Profil</span> est mis à jour en temps réel. Cette option est disponible lorsque vous sélectionnez <b><span class="uicontrol"> Profil authentifié actuel</span></b> ou <b><span class="uicontrol"> Dernier Profil authentifié</span></b>. Lorsque vous utilisez cette option, vous ne pouvez choisir qu’un seul profil authentifié (<span class="keyword"> Audience Manager</span> grise automatiquement les autres). Voir aussi <a href="profile-link-use-case.md"> Cas d'utilisation du graphique de l'unité de liaison de Profil</a>. </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> Graphique des périphériques coopératifs</span></b> </p> </td> 
   <td colname="col2"> <p>Indique à <span class="keyword"> l'Audience Manager</span> de lire les profils du périphérique actuel et jusqu'à 100 autres périphériques en utilisant les liens fournis par l'Experience Cloud Device Co-op</a> <a href="https://docs.adobe.com/content/help/en/device-co-op/using/about/overview.html" format="https" scope="external">. </a></p> <p><span class="keyword"> Device Co-op</span> est une coopérative numérique au sein de laquelle les membres partagent des informations au sujet des liaisons de périphériques. <span class="keyword"> Device Co-op</span> traite ces données dans un graphique <span class="term"> périphérique </span>. Un graphique de périphérique relie les périphériques à des grappes de périphériques. Ces liens sont construits à partir de <a href="https://docs.adobe.com/content/help/en/device-co-op/using/device-graph/links.html" format="https" scope="external"> données probabilistes et déterministes</a>. Les grappes représentent un groupe de dispositifs utilisés par une personne inconnue. <span class="keyword">Device Co-op</span> partage ces grappes avec ses membres afin de les aider à proposer à leurs clients des contenus utiles et cohérents sur tous leurs périphériques. </p> <p> Pour plus d'informations sur <span class="wintitle"> Device Co-op</span>, consultez la section : </p> <p> 
