---
description: Pour créer des règles de fusion , passez en revue et suivez les étapes de chacune des procédures décrites dans cette section.
seo-description: Pour créer des règles de fusion , passez en revue et suivez les étapes de chacune des procédures décrites dans cette section.
seo-title: 'Prise en main des règles de fusion '
solution: Audience Manager
title: 'Prise en main des règles de fusion '
...
# Prise en main des règles de fusion {#getting-started-with-profile-merge-rules}
...
4. Dans le **[!UICONTROL ID Type]** , sélectionnez **[!UICONTROL Cross Device]**.
...
* **[!UICONTROL Data retention for inactive Customer IDs]**: Ce contrôle vous permet de définir la période de rétention des données pour les ID de client inactifs. Cela détermine la durée pendant laquelle  Gestionnaire de  de conserve les ID de client dans notre base de données après leur dernière consultation sur la plate-forme Gestionnaire de de la . La valeur par défaut est de 24 mois (720 jours). La valeur minimale que vous pouvez définir est de 1 mois et la valeur maximale est de 5 ans. Notez que nous comptons tous les mois comme 30 jours.  Gestionnaire de  de exécute un processus qui supprime les ID de client inactifs une fois par semaine, conformément à la rétention de données que vous avez définie pour les ID de client inactifs.

Les champs de texte associés à ces paramètres vous permettent de renommer le [!UICONTROL Data Source] fichier avec un alias qui apparaît dans les options [de la règle de fusion](merge-rule-definitions.md)du. Par exemple, si vous ajoutez un alias à **[!UICONTROL Use as Authenticated Profile]**, ce nom apparaît dans le [!UICONTROL Authenticated Profile Options] . Si vous ajoutez un alias à **[!UICONTROL Use as a Device Graph]**, ce nom apparaît dans le [!UICONTROL Device Options] .

## Création d’une règle de fusion {#create-profile-merge-rule}
...
Vous pouvez créer jusqu’à 3 règles de fusion après avoir configuré une source de données sur plusieurs périphériques. Vous avez accès à une quatrième règle de fusion  ([!UICONTROL All Cross-Device Profiles]) si vous vous abonnez à des destinations [basées sur les](../destinations/people-based-destinations-overview.md)personnes.

Les autorisations d’administrateur sont requises pour créer, modifier ou supprimer une règle. Tous les utilisateurs peuvent  et utiliser des éléments existants [!UICONTROL Profile Merge Rules].
...
>[!TIP]
>
>Voir [Options de règle de fusion définies](merge-rule-definitions.md) pour obtenir des descriptions de ces différents contrôles.
...
## Configuration des règles de fusion {#profile-merge-rule-setup}
...
Vous devez configurer une source [de données](#create-data-source) inter-périphériques et des règles [de fusion de](#create-profile-merge-rule)*avant* d’exécuter ces procédures.

## Pour les clients d’Adobe Experience Platform Identity Service {#id-service-customers}

Il est recommandé d’utiliser la version [!UICONTROL Adobe Experience Platform Identity Service] et la dernière version de [DIL](../../dil/dil-overview.md) lorsque vous travaillez avec [!UICONTROL Profile Merge Rules]. Cependant, vous n’avez pas besoin d’utiliser la [!UICONTROL Adobe Experience Platform Identity Service] pour utiliser cette fonctionnalité. Si vous utilisez simplement [!UICONTROL DIL], reportez-vous à la section [DIL](#legacy-dil) héritée ci-dessous.

### Configuration de la fonction Set Customer ID

Lorsque vous travaillez avec [!UICONTROL Adobe Experience Platform Identity Service], la `setCustomerIDs` fonction transmet les ID déclarés à [!DNL Audience Manager]. Pour utiliser une règle de fusion , vous devez la modifier `setCustomerIDs` pour utiliser le code d’intégration spécifié lors de la création d’une source de données sur plusieurs périphériques. Supposons, par exemple, que vous ayez créé une source de données inter-périphériques avec le code d’intégration `my_datasource_ic`. Pour transmettre un ID déclaré, vous devez ajouter le code d’intégration à la fonction d’ID de, comme illustré dans l’exemple de code modifié ci-dessous.
...
Pour plus d’informations, voir [Création d’une source](#create-data-source) de données sur plusieurs périphériques et d’identifiants [de client, ainsi que d’états](https://marketing.adobe.com/resources/help/en_US/mcvid/?f=mcvid_customer_ids.html)d’authentification.
...
Vérifiez les méthodes de votre [!DNL SDK] code qui vous permettent de passer [!UICONTROL declared IDs] des périphériques [!DNL Android] et [!DNL iOS] mobiles. Les noms des variables pour les bibliothèques [!DNL Android] et [!DNL iOS] de code sont les mêmes :
...
Voir aussi Méthodes du Gestionnaire de  [pour Android](https://marketing.adobe.com/resources/help/en_US/mobile/android/?f=c_audience_manager_methods.html) et Méthodes du Gestionnaire de [pour iOS](https://marketing.adobe.com/resources/help/en_US/mobile/ios/?f=aam_methods.html).
