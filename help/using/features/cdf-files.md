...
Informations de base sur les fichiers [!UICONTROL Customer Data Feed] ([!UICONTROL CDF]) et instructions pour commencer. Commencez ici si vous souhaitez recevoir des fichiers [!UICONTROL CDF] ou simplement plus d’informations.

## Contenu et objectif du fichier {#file-contents-purpose}

Un fichier [!UICONTROL CDF] contient les mêmes données que celles envoyées à nos serveurs par un appel d’événement [!DNL Audience Manager] (`/event`). Cela inclut des données telles que les identifiants utilisateur, [!UICONTROL trait IDs], [!UICONTROL segment IDs] et tous les autres paramètres capturés par un appel d’événement. Les systèmes internes [!DNL Audience Manager] traitent les données d’événement dans un fichier [!UICONTROL CDF] dont le contenu est organisé en champs qui s’affichent dans un ordre défini. [!DNL Audience Manager] tente de générer des fichiers [!UICONTROL CDF] toutes les heures et les stocke dans un compartiment sécurisé spécifique au client sur un serveur [!DNL Amazon S3]. Nous fournissons ces fichiers afin que vous puissiez travailler avec des données [!DNL Audience Manager] en dehors des limites imposées par notre interface utilisateur.
...
>* Vous ne devez pas utiliser les fichiers [!UICONTROL CDF] comme proxy pour surveiller le trafic des pages, réconcilier les incohérences des rapports, ou pour la facturation, etc.

## Prise en main {#getting-started}

Il n&#39;existe aucun processus en libre-service pour démarrer la diffusion de fichiers [!UICONTROL CDF]. Contactez votre consultant [!DNL Audience Manager] ou l’assistance clientèle pour commencer. Pendant la mise en oeuvre, votre représentant [!DNL Audience Manager] :

* Configurez votre compartiment de stockage [!DNL Amazon S3].
* Indiquez des informations d’authentification [!DNL S3] en lecture seule à votre compartiment de stockage de fichiers. Vous ne pourrez pas voir ni accéder aux répertoires et aux fichiers qui appartiennent à d’autres clients.
...
Un fichier [!UICONTROL CDF] comprend certains ou tous les champs définis ci-dessous. Pour plus d’informations sur l’organisation interne des fichiers, voir [Structure de fichier de flux de données client](#cdf-file-structure).
...
<table id="table_46BC897A30C2469AB5911F5B85A3FAA7"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>Il s’agit de l’UUID, l’identifiant utilisateur unique <span class="wintitle"> (UUID), qui est un identifiant d’appareil à 38 chiffres pour le visiteur de votre site. </span> Voir aussi <a href="../reference/ids-in-aam.md"> Index des identifiants en Audience Manager</a>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> Container ID</code> </p> </td> 
   <td colname="col2"> <p>Numérique </p> </td> 
   <td colname="col3"> <p>L’identifiant du conteneur qui déclenche les synchronisations des identifiants. Ce champ n’est renseigné que si vous définissez l’ID de conteneur dans le champ <i>d_nsid</i> de l’implémentation de votre site. Sinon, la valeur par défaut de 0 ne sera pas incluse dans les fichiers CDF. </p> </td> 
  </tr> 
  <tr> 
...
Les données de fichier [!UICONTROL CDF] apparaissent dans l’ordre indiqué ci-dessous.

![](assets/sequence-map.png)

## Identification des tableaux

Les tableaux d’un fichier [!UICONTROL CDF] commencent et se terminent par le séparateur de champ `Ctrl + a`. Cela fait apparaître le premier élément d’un tableau comme un champ de données autonome. Par exemple, le tableau [!UICONTROL traits] réalisé commence par `^A1234`. Le délimiteur de tableau et l’ID `^B5678` suit cette entrée. Par conséquent, vous pouvez être tenté de penser que le premier élément du tableau [!UICONTROL traits] réalisé est l’ID 5678 (car il commence par `^B`). Ce n’est pas le cas, c’est pourquoi vous devez connaître la séquence et la structure d’un fichier de données. Même si le premier élément du tableau [!UICONTROL trait] réalisé (ou de l’un des autres tableaux d’un fichier [!UICONTROL CDF]) commence par `^A`, l’ordre d’apparition ou la position dans le fichier définit le début d’un tableau. De plus, le premier élément d’un tableau est toujours séparé de l’entrée précédente par `^A`.
...
## [!UICONTROL CDF] Nom de fichier : syntaxe et exemple {#cdf-file-name}
...
## [!UICONTROL CDF] Éléments de nom de fichier définis {#cdf-file-name-elements}
...
## [!UICONTROL Customer Data Feed] Les heures de nom de fichier et les heures de contenu de fichier sont différentes {#different-processing-times}
...
Les fichiers [!UICONTROL CDF] enregistrent le temps différemment dans 2 emplacements distincts.
