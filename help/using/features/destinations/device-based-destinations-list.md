---
description: Cette page comprend uniquement les intégrations serveur à serveur actuelles. Pour plus d’informations sur une intégration, consultez sa liste Adobe Exchange (le cas échéant) ou contactez votre conseiller Adobe ou le service d’assistance clientèle.
seo-description: Cette page comprend uniquement les intégrations serveur à serveur actuelles. Pour plus d’informations sur une intégration, consultez sa liste Adobe Exchange (le cas échéant) ou contactez votre conseiller Adobe ou le service d’assistance clientèle.
seo-title: Adobe Audience Manager des destinations serveur à serveur
keywords: activation; activation partners, destination, destinations
solution: Audience Manager
title: Adobe Audience Manager des destinations serveur à serveur
feature: Destination Basics
translation-type: tm+mt
source-git-commit: e05eff3cc04e4a82399752c862e2b2370286f96f
workflow-type: tm+mt
source-wordcount: '1122'
ht-degree: 71%

---


# [!DNL Audience Manager] [!UICONTROL Device-based Destinations]

Cette page inclut les destinations [!DNL Audience Manager] actuelles basées sur les [](/help/using/features/destinations/add-device-based-destinations.md) périphériques uniquement. Dans [!DNL Audience Manager], [!UICONTROL device-based destinations] sont également appelés [!UICONTROL server-to-server destinations]. Pour plus d’informations sur une intégration, consultez sa liste [Adobe Exchange](https://www.adobeexchange.com/experiencecloud.html) (le cas échéant) ou contactez votre [!DNL Adobe] consultant ou le service d’assistance clientèle.

 

* **Dernière mise à jour** = Affiche la dernière date à laquelle il y a eu une mise à jour pour cette [!UICONTROL device-based destination]application.
* **Mises à jour** = Indique si [!DNL Audience Manager] les données sont envoyées en temps réel ou par lot (1 jour) à cette [!UICONTROL destination]fonction.
* **Unsegment** = Unsegmentation décrit les processus qui excluent et suppriment les profils de périphérique [!UICONTROL segments]. Votre capacité à supprimer un profil de périphérique d&#39;un [!UICONTROL segment] périphérique dépend de l&#39;option de périphérique utilisée pour créer un [!UICONTROL Profile Merge Rule]périphérique. La non-segmentation est également utilisée dans le contexte des requêtes de [!DNL GDPR] suppression. Lisez les règles de fusion des [Profils et les processus](/help/using/features/profile-merge-rules/merge-rule-unsegment.md) de désegmentation des périphériques et les partenaires [d&#39;Audience Manager avec des fonctionnalités](/help/using/overview/data-security-and-privacy/data-privacy-requests.md#aam-partners-with-unsegmentation)d&#39;absence de segmentation. [!DNL Audience Manager] envoie les informations de non-segmentation au [!UICONTROL destinations] marqué avec **Oui** ci-dessous.
* **Recevoir des signaux[!DNL IAB TCF]de** consentement = Permet [!DNL Audience Manager Plug-in for IAB TCF] [!DNL IAB TC] de transférer la chaîne à des partenaires d’activation tout en respectant les choix de confidentialité des utilisateurs. Read [Audience Manager Plug-in for IAB TCF](/help/using/overview/data-security-and-privacy/aam-iab-plugin.md).
* **[!DNL ICDS]** = Cette colonne liste les partenaires qui prennent en charge [!UICONTROL Instant Cross-Device Suppression]. Lisez la section sur la suppression [](/help/using/features/profile-merge-rules/instant-cross-device-suppression.md)instantanée de plusieurs périphériques.
* **ID** de périphérique mobile = Cette colonne liste les partenaires qui peuvent recevoir des ID de périphérique mobile [!DNL Audience Manager].

 

| Intégration | Dernière mise à jour | Mises à jour | Dissocier | Réception de signaux de [!DNL IAB TCF] consentement | [!DNL ICDS] | ID de périphérique mobile |
|---------------------------------------------------|------------|-----------|-----------|-----------------------------------|------|-------------------|
| [!DNL 4C Insights] | 26/09/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL 4info] | 09/02/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL AcuityAds] | 09/02/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Adara] | 17/02/2017 | Temps réel | Oui | Non | Non | Non |
| [!DNL Addictive Mobility] | 22/02/2017 | 1xday | Oui | Non | Oui | Oui |
| [!DNL Adelphic] | 22/02/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL AdForm] | 22/02/2017 | Temps réel | Oui | Non | Oui | Oui |
| [!DNL AdGear] | 22/02/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Adikteev] | 26/01/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Adroll] / [!DNL NextRoll] | 10/12/2019 | 1xday | Non | Non | Non | Non |
| [!DNL Adswizz] | 09/10/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Adtheorent] | 22/02/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Affinio] | 03/04/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Aki] | 21/06/2018 | 1xday | Non | Non | Non | Oui |
| [!DNL Amazon Advertising] | 15/06/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL AOL AdLearn] | 02/03/2017 | Temps réel | Non | Non | Non | Oui |
| [!DNL AOL ONE] | 02/03/2017 | Temps réel | Oui | Non | Non | Oui |
| [!DNL AppNexus] | 02/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL AppNexus Yieldex] | 10/08/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Atedra]/[!DNL AdStanding] | 02/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Avocet] | 06/12/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Bidtellect] | 03/03/2017 | Temps réel | Oui | Non | Non | Non |
| [!DNL Beeswax] | 25/05/2018 | 1xday | Non | Non | Non | Oui |
| [!DNL Brandscreen] | 03/03/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Brightroll] | 03/03/2017 | 1xday | Non | Oui | Non | Non |
| [!DNL Button] | 18/12/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL C1 Exchange] | 27/02/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Celtra] | 27/02/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Centro] | 27/02/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL ChoiceStream] | 27/02/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Clearstream.tv] | 27/02/2017 | Temps réel | Oui | Non | Non | Non |
| [!DNL Collective AMP Platform] | 27/02/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Criteo] | 27/02/2017 | Temps réel | Non | Oui | Non | Oui |
| [!DNL Crosswise] | 27/02/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL D.A.C. AudienceOne] | 27/02/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL DataXu] | 11/20/2017 | Temps réel | Non | Non | Non | Oui |
| [!DNL Digilant] | 02/03/2017 | Temps réel |  | Non | Non | Non |
| [!DNL DistrictM] | 02/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL DoubleVerify] | 06/09/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Drawbridge] | 02/03/2017 | Temps réel | Non | Non | Non | Oui |
| [!DNL Dstillery] ([!DNL FKA Media6]) | 02/03/2017 | Temps réel | Oui | Non | Non | Oui |
| [!DNL Dunnhumby] | 21/11/2019 | 1xday | Non | Non | Non | Non |
| [!DNL easyfundraising] | 27/06/2019 | 1xday | Non | Non | Non | Oui |
| [!DNL Epsilon Conversant] | 03/03/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Exponential] | 27/02/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Eyereturn Marketing] | 11/04/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL EyeView Digital] | 14/06/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Fiksu] | 03/03/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Flashtalking] | 02/03/2017 | 1xday | Non | Non | Non | Non |
| [!DNL FreakOut] | 03/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Freewheel] | 27/02/2017 | 1xday | Oui | Oui | Non | Oui |
| [!DNL Gamut Media] | 03/03/2017 | Temps réel |  | Non | Non |  |
| [!DNL Google AdsWords Display] | 02/08/2018 | Temps réel | Oui | Oui | Non | Oui |
| [!DNL Google DoubleClick Bid Manager (DBM)] | 24/04/2017 | Temps réel | Oui | Oui | Oui | Oui |
| [!DNL Google DoubleClick Campaign Manager (DCM fka DFA)] | 24/04/2017 | Temps réel | Oui | Oui | Oui | Oui |
| [!DNL Google DoubleClick for Publishers (DFP, DFPP)] | 24/04/2017 | Temps réel | Oui | Oui | Oui | Oui |
| [!DNL GumGum] | 02/06/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Index Exchange (formerly Casale Media)] | 03/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Infectious Media] | 03/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL inMobi] | 30/03/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Innovid] | 02/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL iPinYou] | 11/07/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Jampp] | 11/05/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Jivox] | 30/08/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Juice Mobile] | 03/03/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Krux] | 30/06/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL Liftoff.io] | 09/11/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL LiveIntent] | 17/07/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL LiveRail] | 20/01/2017 | 1xday |  | Non | Non |  |
| [!DNL Logicad] | 22/06/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Lotame] | 30/06/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL MaxPoint Interactive] | 27/02/2017 | 1xday | Non | Non | Non | Non |
| [!DNL MediaMath] | 27/02/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Microsoft Bing Ads] | 10/10/2019 | Temps réel | Oui | Oui | Oui | Non |
| [!DNL Millennial Media] | 27/02/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL mPlatform China] | 06/08/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Narrative I/O] | 12/01/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Native Touch] | 02/03/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Nativo] | 27/02/2017 | 2x/jour | Oui | Non | Non | Non |
| [!DNL Mail.ru] | 29/04/2019 | 1xday | Oui | Non | Non | Oui |
| [!DNL Mobile Professionals] | 25/05/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Netmining] | 02/03/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Neustar] | 28/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL OnAd] | 13/02/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL OneSpot] | 19/06/2017 | Temps réel | Oui | Non | Non | Oui |
| [!DNL OpenX] | 26/07/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Opera Mediaworks] | 13/02/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Outbrain] | 03/04/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL OwnerIQ] | 13/02/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Platform 161] | 28/11/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL PocketMath] | 22/02/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Powerlinks] | 17/05/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Pubmatic] | 13/02/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Pulsepoint] | 13/02/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL PushSpring] | 28/08/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Quantcast] | 13/02/2017 | Temps réel | Oui | Non | Non | Non |
| [!DNL RhythmOne / RadiumOne] | 13/02/2017 | Temps réel | Oui | Non | Non | Oui |
| [!DNL Remerge] | 31/05/2017 | Temps réel | Oui | Non | Oui | Oui |
| [!DNL Resonate] | 17/02/2017 | 1x/jour | Oui | Non | Non | Non |
| [!DNL RevJet] | 17/02/2017 | 1x/jour | Non | Non | Non | Non |
| [!DNL RocketFuel] | 17/02/2017 | Temps réel | Oui | Non | Oui | Oui |
| [!DNL Rubicon] |  | Temps réel | Oui | Oui | Non |  |
| [!DNL RUN] | 17/02/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Sabio Mobile] | 17/02/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL Simpli.fi] | 17/02/2017 | 1xday | Non | Non | Non | Oui |
| [!DNL Sizmek ad server (Amazon)] | 17/02/2017 | En temps réel | Oui | Non | Non |  |
| [!DNL Slickdeals] | 04/09/2018 | Temps réel | Oui | Non | Non | Oui |
| [!DNL Smart Adserver] | 31/07/2017 | Temps réel | Oui | Non | Oui | Oui |
| [!DNL So-net Media Networks Logicad] |  | 1x/jour | Oui | Non | Non |  |
| [!DNL Sonobi] | 28/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL SpecificMedia] | 03/03/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL SpongeCell] | 03/03/2017 | Temps réel | Oui | Non | Oui | Oui |
| [!DNL SpotX] | 16/03/2018 | 1xday | Non | Oui | Non | Oui |
| [!DNL Stroer] | 03/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL SundaySky] | 03/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL StartApp] | 25/05/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Supership]/[!DNL ScaleOut] | 03/03/2017 | 1x/jour | Oui | Non | Non | Non |
| [!DNL Survata] | 09/08/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL Taboola] | 03/08/2017 | 1xday | Non | Oui | Non | Non |
| [!DNL Tapad] | 03/03/2017 | Temps réel | Non | Non | Non | Oui |
| [!DNL Telaria] | 25/04/2019 | 1xday | Oui | Non | Non | Oui |
| [!DNL Telogical Systems] | 03/03/2017 | En temps réel | Oui | Non | Non | Non |
| [!DNL The Trade Desk] | 03/03/2017 | Temps réel | Oui | Oui | Oui | Oui |
| [!DNL Thnk Realtime] | 05/02/2016 | Temps réel |  | Non | Non |  |
| [!DNL Tremor Video] | 03/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Triple Lift] | 03/03/2017 | Temps réel | Non | Oui | Non | Non |
| [!DNL TrueX] | 11/01/2018 | Temps réel | Oui | Non | Non | Oui |
| [!DNL TubeMogul] | 03/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Turn] / [!DNL Amobee] | 03/03/2027 | Temps réel | Non | Non | Non | Oui |
| [!DNL Twenga] | 29/10/2018 | Temps réel | Oui | Non | Non | Non |
| [!DNL Twitter Tailored Audiences] | 27/03/2017 | Temps réel | Oui | Non | Oui | Non |
| [!DNL Undertone] | 11/07/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Verve] | 15/06/2017 | 1xday | Oui | Non | Non | Oui |
| [!DNL VideoAmp] | 03/03/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Videology] | 03/03/2017 | 1xday | Non | Non | Non | Non |
| [!DNL VisualIQ] | 24/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Wikibuy] | 04/08/2018 | Temps réel | Oui | Non | Non | Oui |
| [!DNL X+1] | 05/02/2016 | 1xday |  | Non | Non |  |
| [!DNL Xaxis] | 03/03/2017 | 1xday | Oui | Non | Non | Non |
| [!DNL Yahoo DataX] | 03/03/2017 | Temps réel | Oui | Oui | Oui | Oui |
| [!DNL Yahoo! Japan] | 03/03/2017 | Temps réel | Non | Non | Non | Non |
| [!DNL Yandex] | 28/11/2018 | 1xday | Oui | Non | Non | Oui |
| [!DNL Yieldex] | 21/08/2017 | Varie | Oui | Non | Non | Non |
| [!DNL YieldMo] | 03/03/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Yume] | 31/05/2017 | 1xday | Non | Non | Non | Non |
| [!DNL Zeta] / [!DNL Sizmek DSP] / [!DNL RocketFuel] | 17/02/2017 | Temps réel | Oui | Non | Oui | Oui |