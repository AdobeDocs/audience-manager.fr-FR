---
description: Recherchez les signaux utilisés et inutilisés reçus pendant des périodes spécifiques. Pour rechercher des signaux, sélectionnez Données d'audience > Signaux > Rechercher.
seo-description: Recherchez les signaux utilisés et inutilisés reçus pendant des périodes spécifiques. Pour rechercher des signaux, sélectionnez Données d'audience > Signaux > Rechercher.
seo-title: Signale la recherche
title: Signale la recherche
translation-type: tm+mt
source-git-commit: 16c0dd83c18e720045995ac1851b4c91f3346183

---


# Signals Search Overview {#signals-search}

Recherchez les signaux utilisés et inutilisés reçus pendant des périodes spécifiques. Pour rechercher des signaux, sélectionnez Données d'audience &gt; Signaux &gt; Rechercher.

* [Rechercher des signaux par paires clé-valeur](/help/using/features/data-explorer/data-explorer-signals-search/data-explorer-search-pairs.md)
* [Recherche de signaux par filtres](/help/using/features/data-explorer/data-explorer-signals-search/data-explorer-search-filters.md)
* [Recherche avancée d'Adobe Analytics](/help/using/features/data-explorer/data-explorer-signals-search/data-explorer-search-analytics.md)
* [Filtrage des résultats de recherche](/help/using/features/data-explorer/data-explorer-signals-search/data-explorer-filter-results.md)
* [Enregistrer les critères de recherche](/help/using/features/data-explorer/data-explorer-signals-search/data-explorer-save-search.md)



