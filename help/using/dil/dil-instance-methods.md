---
description: Les API DIL au niveau de l’instance vous permettent de créer et d’utiliser des objets Audience Manager par programmation. Les méthodes au niveau de l’instance améliorent la fonctionnalité d’API établie par les méthodes au niveau de la classe .
keywords: créer une caractéristique;créer une caractéristique
...
Les API [!UICONTROL DIL] au niveau de l’instance vous permettent de créer et d’utiliser des objets Audience Manager par programmation. Les méthodes au niveau de l’instance améliorent la fonctionnalité d’API établie par les méthodes au niveau de la classe .
...
Lorsque vous utilisez les API [!UICONTROL DIL] au niveau de l’instance :
...
| `prefix` | Chaîne | Facultatif. La valeur de chaîne précédée de chaque clé d’objet (remplace la clé d’origine). |
| `return` | DIL.api | Renvoie l’objet API de l’instance DIL active. |

**Réponse**

Renvoie l’objet API de l’instance de [!UICONTROL DIL] active.
...
Envoie toutes les données en attente à Audience Manager pour l’instance [!UICONTROL DIL].
...
>[!NOTE]
>
>Vous pouvez chaîner d’autres appels API à cette méthode. En outre, [!UICONTROL DIL] écrit des données codées dans un cookie de destination. Par exemple, les espaces sont codés en `%20` et les points-virgules en `%3B`.
...
| Nom | Type | Description |
|---|---|---|
| `fn` | Fonction | La fonction que vous souhaitez exécuter après le traitement JSON est traitée par le rappel par défaut qui gère la publication de destination. |

**Réponse**

Renvoie un objet API de l’instance de [!UICONTROL DIL] active.
...
Retourne l’état de l’instance [!UICONTROL DIL] courante. Utile pour le débogage et le dépannage.
...
<table id="table_ADC7501511914805A6A6B24B2DFEBA51"> 
 ...
 <tbody> 
  <tr valign="top"> 
   ...
   <td colname="col2"> <p>Entre différents partenaires de données et Audience Manager. Par exemple, le partenaire x l’utilisera pour synchroniser un identifiant utilisateur avec le partenaire y, puis l’enverra à Audience Manager. </p> <p> <p><b>Important :</b> cette méthode est obsolète. Utilisez la méthode <code> idSyncByURL </code> de l’instance Adobe Experience Platform Identity Service. </p> </p> </td> 
  </tr> 
  <tr valign="top"> 
   <td colname="col1"> <code> dil.Instance.api.aamIdSync(initConfig) </code> </td> 
   <td colname="col2"> <p>Lorsque vous connaissez déjà l’identifiant utilisateur et que vous souhaitez l’envoyer à Audience Manager. </p> <p> <p><b>Important :</b> cette méthode est obsolète. Utilisez la méthode <code> idSyncByDataSource </code> de l’instance Adobe Experience Platform Identity Service. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* **`%TIMESTAMP%`:** génère une date et une heure (en millisecondes). Utilisé pour la mise en cache.
* **`%DID%`:** permet d’insérer l’Audience Manager ID de l’utilisateur.
* **`%HTTP_PROTO%`:** définit le protocole de page ( `http` ou `https`).

**Réponse**

Les deux fonctions renvoient une `Successfully queued` en cas de réussite. Autrement, elles renvoient une chaîne contenant un message d’erreur.
...
* Lorsqu’il est `secureDataCollection= true` (par défaut), [!UICONTROL DIL] effectue toujours des appels HTTPS sécurisés.

* Lorsqu’il est `secureDataCollection= false`, [!UICONTROL DIL] effectue des appels HTTP ou HTTPS en suivant le protocole de sécurité défini par la page.

>[!IMPORTANT]
>
>Définissez `secureDataCollection= false` si vous utilisez visitorAPI.js et [!UICONTROL DIL] sur la même page. Voir l’exemple de code ci-dessous.
...
`useCORSOnly` paramètre booléen true/false qui contrôle la manière dont le navigateur demande des ressources à d’autres domaines.
...
>[!IMPORTANT]
>
>* Nous vous recommandons de définir `useCORSOnly: true` uniquement lorsque vous êtes sûr que les visiteurs de votre site disposent de navigateurs qui prennent en charge cette fonctionnalité.
>* Lorsqu’il est `useCORSOnly: true`, [!UICONTROL DIL] n’effectue pas d’appels d’ID depuis Internet Explorer version 9 ou antérieure.
>

## useImageRequest {#useimagerequest}

Modifie le type de requête en `<img>` d’image à partir du `<src>` de script.
...
>* [Fonctions de synchronisation dans Adobe Experience Platform Identity Service](https://experienceleague.adobe.com/docs/id-service/using/id-service-api/methods/idsync.html?lang=fr)
>* [Création DIL](../dil/dil-class-overview/dil-create.md#dil-create)
>* [Adobe Experience Platform Identity Service : useCORSOnly](https://experienceleague.adobe.com/docs/id-service/using/id-service-api/configurations/use-cors-only.html?lang=fr)
>* [Prise en charge de la norme CORS dans Adobe Experience Platform Identity Service](https://experienceleague.adobe.com/docs/id-service/using/reference/cors.html?lang=fr)
