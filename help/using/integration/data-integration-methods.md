---
description: Présentation générale de la manière dont Audience Manager exchange des informations avec d’autres fournisseurs et systèmes de données.
...
Présentation générale de la manière dont Audience Manager exchange des informations avec d’autres fournisseurs et systèmes de données.

## Méthodes d’intégration des données prises en charge : temps réel et [!DNL Server-to-Server] {#supported-methods}

Le choix de la bonne méthode d’intégration dépend de la combinaison des exigences de l’entreprise et des fonctionnalités techniques de votre partenaire de données. Audience Manager exchange les informations sur les visiteurs à d’autres fournisseurs de données en utilisant l’une des méthodes suivantes :

* **Temps réel :** transfère immédiatement les données lorsqu’un utilisateur visite votre site. Cette méthode est également connue sous le nom d’intégration *`synchronous`*.
* **Lot ([!DNL Server-to-Server]) :** Transfère des données entre les serveurs selon une planification définie une fois qu’un visiteur a quitté la page. Cette méthode est également connue sous le nom d’intégration *`out-of-band`* ou *`asynchronous`*.

## Conditions préalables : création d’une taxonomie des caractéristiques {#prereqs}

Avant que le processus d’intégration ne commence, pensez à [créer des caractéristiques](../features/traits/create-onboarded-rule-based-traits.md) et à une [structure de dossiers](../features/traits/trait-storage.md#create-trait-storage-folder) dans l’interface utilisateur de [!DNL Audience Manager]. La taxonomie contiendra tous vos [!UICONTROL traits] organisés dans une hiérarchie logique.
...
Présentation générale de la manière dont Audience Manager effectue un exchange de données synchrone avec un fournisseur tiers.
...
### Procédure d’intégration des données en temps réel
...
1. Audience Manager charge un Iframe et effectue un appel à [!UICONTROL Data Collection Server] ([!DNL DCS]).
...
Présentation générale de la manière dont Audience Manager exchange les données de manière synchrone (en temps réel) avec un fournisseur tiers.
...
1. Une requête planifiée, généralement sur un intervalle quotidien, exchange les données de segmentation des visiteurs entre l’Audience Manager et votre fournisseur de données tiers.

![](assets/s2s_70.png)

Pour plus d’informations sur les délais pendant lesquels l’Audience Manager traite les transferts de fichiers [!DNL Server-to-Server] ([!UICONTROL S2S]) entrants et sortants, voir [ Instructions sur le délai de création de rapports et de transfert de fichiers](../reference/reporting-file-transfer-timeframe.md).
