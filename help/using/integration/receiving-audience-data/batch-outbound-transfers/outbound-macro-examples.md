...
seo-description: Exemples d’utilisation de macros courantes pour créer des modèles de fichiers sortants.
...
Pour vous aider à comprendre comment la macro `DPUUID` génère des données, supposons que nous avons 2 `DPID`mappés sur `DPUUID`comme illustré ci-dessous :

* Le DPID `1111` correspond aux DPUUID `AAAA` (horodatage = 1) et `BBBB` (horodatage = 2).
* Le DPID `2222` correspond à DPUUID `CCCC`.
...
<table id="table_6A6D94F994C1475BB09126BA0B815B1F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p> <code> [["1111","BBBB"],["2222","CCCC"]] </code> </p> <p>Pour le DPID <code> 1111 </code>, la macro correspond à DPUUID <code> BBBB </code> uniquement parce que cet ID possède un horodatage plus grand. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col3"> <p> <code> [["2222","CCCC"]] </code> </p> <p>Bien que <code> maxMappings=2 </code>, cette macro renvoie uniquement un mappage DPID vers DPUUID, car le DPID spécifié n’a qu’un seul DPUUID. </p> </td> 
  </tr> 
 </tbody> 
</table>
